...
심볼릭 링크는 앞서 말했듯이 하드 링크의 두 가지 단점을 보완하기 위해 탄생된 것이다. 즉 하드 링크는 물리적인 장치들을 포괄하지 못하고 파일만 참조할 수 있지 ==디렉토리를 참조할 수 없다는 점==이다. 심볼릭 링크는 파일이나 디렉토리를 가리킬 텍스트 포인터를 가지고 있는 특수한 형태의 파일이다.
