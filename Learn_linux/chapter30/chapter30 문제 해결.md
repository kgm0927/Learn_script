...
### 이제는 스크립트를 좀 더 복잡하게 만들어, 실수를 했을대나 프로그램이 원치 않는 동작을 할 때 무슨 일이 벌어지는지 살펴볼 차례다. 이 장에서는 스크립트에서 발생하는 흔한 오류들과 문제를 찾아 해결하는 방법들을 살펴볼 예정이다.
...
또한 bash는 **-x** 옵션이나 [[set]] 명령어에 -x 옵션으로 트레이싱 방법을 제공한다. 우리는 초기의 문제 있는 스크립트에 -x 옵션을 첫 줄에 추가하여 스크립트 전체를 추적할 수 있다.
