...
패키지 선언을 한 다음에 Record 자료형을 정의한다. <mark style="background: #ADCCFFA6;">프로토콜 버퍼에서는 repeated 키워드를 사용해 자료형의 슬라이스를 정의한다.</mark> 따라서 repeated Record는 records 필드가 go 코드로는 `[]Record` 자료형이라는 의미이다.
